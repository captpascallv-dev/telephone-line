...
if (-not [IO.File]::Exists([string]$config.executable)) { throw "Host executable does not exist: $($config.executable)" }
if (-not [IO.Directory]::Exists([string]$config.working_directory)) { throw "Host working directory does not exist: $($config.working_directory)" }
