...
Use this sequence as the guaranteed normal entry. Expert lower-level scripts (`Start-TelephoneLineJob.ps1` without a supervisor run, `Resume-TelephoneLines.ps1`, adapter `-Operation recover`, and caller-supplied launchers) remain valid compatible bindings; they are not the first-use path.

1. **Install** the public tree with `src/install/Install-TelephoneLine.ps1`. Point the Lead CLI at an explicit independent executable (`TELEPHONE_LINE_STABLE_CLI` or `stable-cli-policy.json`). Do not glob App version directories, download a runtime, or switch account/model. Install registers the per-user wired supervisor task.
2. **Supervisor-owned initial Lead/run**: create a valid `telephone-line-wired-supervisor-request-v1` whose command is `Start-TelephoneLineJob.ps1` (or the frozen Lead launcher the run host should execute). Publish it with `src/supervisor/Start-TelephoneWiredRun.ps1`. The supervisor claims the inbox item, writes a durable host-start intent, and starts `Invoke-TelephoneSupervisorRunHost.ps1` with `TELEPHONE_LINE_SUPERVISOR_RUN_ID`. That run host is the owner of the initial Lead/run.
3. **Dispatch under that owner**: the run host launches the request command. `Start-TelephoneLineJob.ps1` still publishes durable job intent, registers the resolved StateRoot plus line/route/Lead/project/run identity for new and reused dashboard watchers, and records supervisor lineage when the inherited run id is present. The Lead exits immediately (`lead_should_exit_now=true`).
4. **Original callback**: when `receipt.json` exists, the collector wakes the exact frozen Lead session using a durable receipt/session/wake key. A proven same-key replay does not start a second executor or a second automatic callback. A verified pre-native-turn active-writer conflict stays eligible only after the exact bound writer is released; missing writer identity is UNKNOWN and is not treated as released. After that writer releases, the same key may retry without rerunning the executor. Ambiguous acceptance does not auto-resend. Trusted manual consumption of an already-read receipt is a distinct, idempotent closeout and is never recorded as automatic callback success.
5. **Observer**: `Ensure-TelephoneDashboard.ps1` starts or reuses one watcher. New and reused watchers consume the durable source registry. The global cockpit, when configured as the ensure override, reads the same line-sources file. The read-only dashboard does not dispatch, retry, or judge PASS.

For new work, wired supervisor inbox is the recommended first-use route after install. `Start-TelephoneLineJob.ps1` remains the dispatch the run host invokes and a compatible low-level entry when a supervisor run id is already inherited. The App Server binding in [codex-app-server-lead.md](codex-app-server-lead.md) remains a supported wireless alternative after its first turn is accepted. Do not switch an already accepted wireless thread to wired transport; recover the same exact App Server thread instead. If the original Lead turn is still active when the receipt returns, Telephone keeps one pending callback and delivers it after that same thread is idle.
...
If Doctor reports `SUPERVISOR_INSTALL_VIEW_MISMATCH`, follow that installed-task diagnosis: bind later commands to the physical install already verified for this machine (`TELEPHONE_LINE_INSTALL_ROOT` or `-InstallRoot`). Do not copy another machine's absolute path, and do not assume `%LOCALAPPDATA%\TelephoneLine` is the scheduler view when the views differ.
