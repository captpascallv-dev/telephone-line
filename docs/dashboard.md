...
Displayed progression is the **current** workstream. Ended interrupts or failures that a later live successor on the same worktree or session has already replaced leave the regular lists, activity/fault/todo counts, overall verdict, and “does the operator need to act” prompts. Raw UNKNOWN records stay on disk for diagnosis; this observer does not add a history panel. Unstarted recovery, a current-task failure, a stuck callback, duplicate live owners, and independent parallel failures stay visible. A later file timestamp or a new file existing is not enough to paint the current view green.
...
An old receipt plus relay-error with no delivery leaves the evidence on disk. Exact public successor Lead/binding/job identity or exact terminal proof removes that lineage from the current view. A wrong project, session, run, or job, an unbound successor, or missing terminal proof stays yellow. A successful job (receipt with delivery and no relay-error) is not retired merely because a later job exists. `BATCH_COLLECTING` is informational: it is not a current-error finding and does not keep a group visible after exact terminal retirement.
