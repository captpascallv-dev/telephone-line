...
It excludes `.control/`, `.git/`, `node_modules/`, `logs/`, `receipts/`, `dispatches/`, `deliveries/`, `cache/`, `caches/`, `state/`, installer images, binaries outside `src/cockpit/runtime/win-x64/`, and any path taken from a user profile. The cockpit runtime folder is the only allowed `.exe`/`.dll` tree. `.control` is private control plane and must not appear in an archive, in the manifest `files` list, or in any packaging artifact. Shipped Cursor external Lead examples under `docs/examples/cursor-external-lead/` and Codex app-server Lead examples under `docs/examples/codex-app-server-lead/` are placeholders only.
...
`release-manifest.json` uses protocol `telephone-line-release-manifest-v1`. It records product `telephone-line`, version matching `src/packaging/TelephonePackaging.Common.ps1`, license `MPL-2.0`, platform `windows`, denominator 8, and the eight `route_id` values read from `src/catalog/routes.json`. Artifact identity fields stay null in this repository file so regeneration does not depend on a zip that is not stored here. Every `files` row is a redistributable path with bytes and SHA-256; `release-manifest.json` is omitted from that list so regeneration remains stable. Regenerating over an unchanged tree is byte-identical (`manifest_deterministic`). The file validates through `Get-TelephoneSchemaPath -Name release-manifest` (`manifest_schema_valid`). A published GitHub Release may attach a separate asset manifest whose artifact fields name the zip bytes and SHA-256.
