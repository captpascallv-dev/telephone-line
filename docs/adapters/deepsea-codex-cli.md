...
Shared Headless uses `Session.eventAt(SessionSeq(seq))`. The contained profile disables `web-fetch-http` whenever `web` is disabled. Failures keep a bounded known-category diagnostic on the receipt (`waiting for service: web` or a numeric exit class); unknown stream tokens are omitted and hashed.
